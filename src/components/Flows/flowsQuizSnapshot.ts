...
export function loadQuizAnswersFromStorage (): {
  ...
  try {
    ...
    if (!raw) {
      return { answers: {}, quizCompletedAt: null, selectedContextIds: [] }
    }
...
export function readFlowsQuizSnapshot (): FlowsQuizSnapshot {
  const { answers, quizCompletedAt, selectedContextIds } = loadQuizAnswersFromStorage()
  ...
  const percents = computeOverallPercents(answers, selectedContextIds)
  if (!percents) {
